  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "var(--space-xl) var(--space-m)" }}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:"var(--space-s)",marginBottom:"var(--space-xl)",padding:"var(--space-l)",background:"var(--color-bg-secondary)",borderRadius:"var(--radius-s)",border:"1px solid var(--color-border-light)"}}>
        <div style={{textAlign:"center",padding:"var(--space-m)"}}><div style={{fontSize:"var(--text-xxl)",fontWeight:900,color:"var(--color-accent)",fontFamily:"var(--font-heading)"}}>{servers.length}</div><div style={{fontSize:"var(--text-xs)",color:"var(--color-text-tertiary)",marginTop:4}}>Всего серверов</div></div><div style={{textAlign:"center",padding:"var(--space-m)"}}><div style={{fontSize:"var(--text-xxl)",fontWeight:900,color:"#065f46",fontFamily:"var(--font-heading)"}}>{servers.filter((s:any)=>s.russianDocs).length}</div><div style={{fontSize:"var(--text-xs)",color:"var(--color-text-tertiary)",marginTop:4}}>С русской док.</div></div><div style={{textAlign:"center",padding:"var(--space-m)"}}><div style={{fontSize:"var(--text-xxl)",fontWeight:900,color:"#1e40af",fontFamily:"var(--font-heading)"}}>{servers.filter((s:any)=>!s.requiresApiKey).length}</div><div style={{fontSize:"var(--text-xs)",color:"var(--color-text-tertiary)",marginTop:4}}>Без VPN / API</div></div><div style={{textAlign:"center",padding:"var(--space-m)"}}><div style={{fontSize:"var(--text-xxl)",fontWeight:900,color:"var(--color-text-primary)",fontFamily:"var(--font-heading)"}}>{new Set(servers.map((s:any)=>s.category)).size}</div><div style={{fontSize:"var(--text-xs)",color:"var(--color-text-tertiary)",marginTop:4}}>Категорий</div></div><div style={{textAlign:"center",padding:"var(--space-m)"}}><div style={{fontSize:"var(--text-xxl)",fontWeight:900,color:"var(--color-warning)",fontFamily:"var(--font-heading)"}}>{servers.filter((s:any)=>s.isFeatured).length}</div><div style={{fontSize:"var(--text-xs)",color:"var(--color-text-tertiary)",marginTop:4}}>Избранных</div></div>
      </div>
