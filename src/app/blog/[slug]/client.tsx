...
export default function PostPageClient({ post }: any) {
  ...
  async function submitComment() {
    ...
    await fetch("/api/blog/comment", {
      ...
      body: JSON.stringify({ ...commentForm, postId: post.id }),
    });
    setSubmitting(false);
    setSubmitted(true);
  }
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "var(--space-xl) var(--space-m)" }}>
      ...
      {/* Content */}
      <div style={{
        fontSize: "var(--text-s)", lineHeight: 1.9, color: "var(--color-text-primary)",
        whiteSpace: "pre-wrap", marginBottom: "var(--space-xl)",
      }}>
        {post.content}
      </div>
      ...
      <div style={{ borderTop: "1px solid var(--color-border)", paddingTop: "var(--space-xl)" }}>
        <h2 style={{ fontSize: "var(--text-l)", fontWeight: 700, marginBottom: "var(--space-l)" }}>
          ...
          <div style={{ marginTop: "var(--space-l)" }}>
            {!showCommentForm ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-s)" }}>
                <input value={commentForm.authorName} onChange={e => setCommentForm({ ...commentForm, authorName: e.target.value })}
                  placeholder="Ваше имя *" style={{ padding: "10px 12px", borderRadius: "var(--radius-s)", border: "1px solid var(--color-border)", outline: "none", fontSize: "var(--text-s)" }} />
                <input value={commentForm.authorEmail} onChange={e => setCommentForm({ ...commentForm, authorEmail: e.target.value })}
                  placeholder="Email (не публикуется)" style={{ padding: "10px 12px", borderRadius: "var(--radius-s)", border: "1px solid var(--color-border)", outline: "none", fontSize: "var(--text-s)" }} />
                <textarea value={commentForm.content} onChange={e => setCommentForm({ ...commentForm, content: e.target.value })}
                  placeholder="Ваш комментарий *" rows={4} style={{ padding: "10px 12px", borderRadius: "var(--radius-s)", border: "1px solid var(--color-border)", outline: "none", fontSize: "var(--text-s)", resize: "vertical" }} />
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={submitComment} disabled={submitting || !commentForm.authorName || !commentForm.content}
                    style={{ display: "flex", alignItems: "center", gap: 4, padding: "10px 20px", borderRadius: "var(--radius-m)", background: "var(--color-accent)", color: "white", border: "none", fontSize: "var(--text-s)", fontWeight: 600, cursor: "pointer" }}>
