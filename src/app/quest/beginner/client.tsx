...
const STEPS = [
  ...
  },
  {
    ...
    detail: "Сейчас произойдёт магия. Ты напишешь обычный текст — и через 5 минут увидишь работающий сайт. Не веришь? Давай проверим.\n\nПредставь: ты фрилансер. Клиент — мастерская по ремонту телефонов. Ему нужен сайт. Бюджет небольшой, срок — вчера. Раньше ты бы потратил неделю на вёрстку. Сейчас — 5 минут и один промпт.\n\nОткрой Cursor → Ctrl+I → вставь промпт ниже → смотри как рождается сайт.",
    checklist: [
  ...
  },
  {
    ...
    time: "10 мин",
    color: "var(--color-info)",
    detail: "Один программист забыл убрать API-ключ из кода и залил на GitHub. Через час пришёл счёт на 5000 долларов — кто-то использовал его ключ для майнинга крипты.\n\n.env — это сейф для секретов. Здесь лежат: пароли, ключи API, токены. Этот файл НИКОГДА не попадает в GitHub. .gitignore — это охранник, который не пускает .env в репозиторий.\n\n5 минут настройки сейчас = тысячи долларов экономии потом.",
    checklist: [
