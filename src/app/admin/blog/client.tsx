...
export default function BlogAdminClient({ posts, categories, authors, pendingComments, total, page, perPage, currentAuthor, editPostId }: any) {
  ...
  async function save() {
    ...
    if (!form.slug) form.slug = form.title.toLowerCase().split("").map((c: any) => ({а:"a",б:"b",в:"v",г:"g",д:"d",е:"e",ё:"yo",ж:"zh",з:"z",и:"i",й:"y",к:"k",л:"l",м:"m",н:"n",о:"o",п:"p",р:"r",с:"s",т:"t",у:"u",ф:"f",х:"h",ц:"ts",ч:"ch",ш:"sh",щ:"sch",ъ:"",ы:"y",ь:"",э:"e",ю:"yu",я:"ya"," ":"-"})[c] || (c.match(/[a-z0-9-]/) ? c : "")).join("").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "post";
