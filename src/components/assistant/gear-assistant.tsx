...
export default function GearAssistant({
  context = "", isLoggedIn = false, isPro = false, hints = [],
}: { context?: string; isLoggedIn?: boolean; isPro?: boolean; hints?: string[] }) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      setPupilOffset({ x: (dx / (dist || 1)) * factor * 2.5, y: (dy / (dist || 1)) * factor * 2.5 });
    };
  ...
  useEffect(() => {
    function scheduleBlink() {
      ...
      return setTimeout(() => { setBlinking(true); setTimeout(() => setBlinking(false), 200); scheduleBlink(); }, delay);
    }
    const t = scheduleBlink(); return () => clearTimeout(t);
  }, []);

  // Idle timer
  useEffect(() => { const t = setInterval(() => setIdleTimer(prev => prev + 1), 1000); return () => clearInterval(t); }, []);

  // Proactive hints every 20 seconds (if not in chat)
  useEffect(() => {
    if (showChat || showBubble || !isPro || hints.length === 0) return;
    if (idleTimer > 0 && idleTimer % 25 === 0) {
  ...
  // Initial greeting after 45 seconds
  useEffect(() => {
    if (idleTimer === 45 && state === "idle" && !showBubble && isPro && hints.length > 0) {
      setMessage(hints[0]);
      setShowBubble(true);
      setHintIndex(1);
      setTimeout(() => setShowBubble(false), 8000);
  ...
  async function ask(question: string) {
    ...
    setState("thinking"); setHistory(prev => [...prev, { text: question, type: "user" }]); setUserInput("");
    try {
      ...
      const answer = data.response || "Хм, я пока не знаю ответа.";
      setMessage(answer); setHistory(prev => [...prev, { text: answer, type: "assistant" }]);
      setState("responding"); setTimeout(() => setState("idle"), 3000);
    } catch {
      setMessage("Ой, что-то пошло не так."); setState("responding"); setTimeout(() => setState("idle"), 3000);
    }
  }
  ...
  function toggleChat() {
    if (!isPro) {
      ...
      setShowBubble(true); setTimeout(() => setShowBubble(false), 6000);
      return;
    }
...
function GearIcon({ size, color, spinning, blinking, pupilOffset }: { size: number; color?: string; spinning?: boolean; blinking?: boolean; pupilOffset: { x: number; y: number } }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={color || "currentColor"} style={{ animation: spinning ? "spin 2s linear infinite" : "none" }}>
      ...
      {blinking && (<><rect x="5" y="5.5" width="8" height="5" fill={color || "#0fb880"} rx="2" /><rect x="11" y="5.5" width="8" height="5" fill={color || "#0fb880"} rx="2" /></>)}
