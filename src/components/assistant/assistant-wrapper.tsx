...
export default async function AssistantWrapper() {
  ...
  try {
    ...
    if (pathname.includes("/corporate-website") || pathname.includes("/saas-project") || pathname.includes("/game-dev")) {
      ...
      hints = [
        "ProektMap ведёт от цели до работающего AI-продукта через решения, артефакты и проверки.",
        "Начни с хаба «Готовые решения AI» — первый проверенный маршрут помогает запустить SaaS.",
