...
async function seed() {
  const rels = [
    { sourceType:"mcp", sourceSlug:"firecrawl-mcp", targetType:"pattern", targetSlug:"ai-seo-auditor", relType:"uses", weight:2 },
  ...
  let count = 0;
  for (const r of rels) {
    await p.relation.upsert({
      where: { sourceType_sourceSlug_targetType_targetSlug: r },
      update: r,
      create: r
    }).then(() => count++).catch(() => {});
  }
  console.log("Seeded", count, "relations");
  await p.$disconnect();
}
seed();
