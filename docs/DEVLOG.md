...
## 28.08.2026 — Карта навыков Skill, Глоссарий вайбкодинга, 14 категорий AI-инструментов, SEO-валидатор и Behance-портфолио

### 1. Карта навыков Skill (`/skills` и `/skills/[slug]`)
- Разработана архитектура профессионального роста создателя цифровых продуктов: 5 Apex-доменов, L1-L3 уровни, Proof of Work артефакты.
- Интерактивный радар компетенций, связка с готовыми решениями (`/resheniya`).
- Документация: `docs/CAPABILITY-MAP-V1.md`.

### 2. Глоссарий сленга вайбкодинга и AI-инженерии (`/glossary`)
- 28+ терминов с простыми объяснениями, аналогиями и примерами промптов.
- Интерактивные бейджи-подсказки в описаниях навыков.
- Кнопки быстрого шеринга в Telegram, VK, WhatsApp и копирование ссылки.
- Поддержка кастомных emoji-иконок в админ-меню.

### 3. Классификация AI-инструментов на 14 категорий (`/ai-tools`)
- Добавлена классификация: LLM, AI-кодинг, AI-агенты, Генерация картинок/видео/аудио, Local AI, Маркетинг и др.
- Обновлена модель `AiTool` в Prisma, фильтры в каталоге и админке `/admin/ai-tools`.

### 4. Автоматическая SEO-проверка и валидация Sitemap
- Скрипт `scripts/validate-sitemap.ts` и команда `npm run validate:sitemap`.
- Правило `.cursor/rules/sitemap-seo.mdc` для обязательной регистрации всех страниц в `SITE_TREE` и `sitemap.ts`.

### 5. Социальное портфолио вайбкодеров в стиле Behance
- Модели `AiProject` и `ProjectLike` (связь с пользователями, галерея скриншотов, AI-рецепты, лайки/респекты, премодерация).
- Страница добавления работы (`/projects/new`) с начислением +150 XP.
- Личный кабинет (`/dashboard`): настройка профиля вайбкодера, управление работами и статистика.
...
## 29.08.2026 — Готовые секции и виджеты для сайта (UI-Атлас), 5-слойная библиотека AI-паттернов, Admin-панель и Skill ui-pattern-author

### 1. Новый раздел «Готовые секции и виджеты для сайта» (`/ui-patterns`)
- Разработана и запущена интерактивная библиотека из 21 эталонного UI-паттерна со строгой геометрией ProektMap (0px radius).
- Полная 5-слойная структура для каждого паттерна:
  1. **Live Studio Sandbox:** интерактивная песочница с переключением вьюпортов (Desktop / Tablet / Mobile), сеткой и живыми действиями.
  2. **Анатомия вёрстки:** CSS-правила, физика анимаций и z-index изоляция.
  3. **Слой WHY:** разбор инженерных решений, последствий ошибок и влияние на UX/стабильность.
  4. **Master-промпты:** готовые формулировки для Cursor Composer, v0, Lovable и Claude с защитными Negative Prompts.
  5. **Production-Ready Code:** типобезопасные компоненты на TypeScript и Next.js.
- Добавлен раздел «Дизайн-Рецепты» (`/ui-patterns/recipes`) для сборки экранов под ключ.

### 2. Admin-управление UI-паттернами (`/admin/ui-patterns`)
- Модель `UiPatternMeta` в БД: хранение кастомных скриншотов, заголовков, описаний и статусов Free/Pro.
- Админка с поиском, фильтрацией, загрузкой скриншотов через `/api/upload` и мгновенным сохранением через API `/api/admin/ui-patterns`.
- Пункт «Секции и виджеты» добавлен в боковое меню админки.

### 3. Главная страница и экосистема
- Виджет «Готовые секции и виджеты для сайта» на главной странице (`/`) с автоматической сортировкой (новые всегда первые), обложками карточек и динамическим счетчиком.
- Добавлен пункт в шапку меню под разделом «Песочница» и карточка в Hub-странице `/sandbox`.
- Полная SEO-интеграция: регистрация в `SITE_TREE`, генерация динамических путей в `sitemap.ts`, canonical-метаданные.

### 4. Автономный AI Skill (`ui-pattern-author`)
- Создан Skill `.cursor/skills/ui-pattern-author/SKILL.md` для автоматического создания новых 5-слойных паттернов по сырому коду или референсу.
- Протестирован и успешно внедрен паттерн `floating-pulse-button` («Пульсирующая фиксированная кнопка действия»).

### Итог
- Сборка `rm -rf .next && npx next build` завершена без ошибок.
- Валидация `npm run validate:sitemap` пройдена успешно (0 ошибок).
- PM2 перезапущен. Live: `/`, `/ui-patterns`, `/ui-patterns/floating-pulse-button`, `/sandbox`, `/admin/ui-patterns` (HTTP 200).

### День закрыт
DEVLOG + commit + push. `main` = `master`.

## 30.08.2026 — Хаб микросервисов (/services), Avito Photo Lab (pHash + RGB EMD + EXIF Spoofer + XML Feeds), Готовое решение «AI-магазин на Авито» (/resheniya/avito-business) и архитектурная связка «Микросервисы + Решения»

### 1. Архитектурная идея и концепт: Связка «Микросервисы + Готовые решения» для будущих проектов и AI-агентов
- **Проблема классических платформ:** Утилиты живут отдельно как разрозненные одноразовые тулзы, курсы/инструкции оторваны от практики, а каталоги сервисов — просто сухие списки ссылок.
- **Инженерная синергия ProektMap:**
  1. **Клиентские микросервисы (`/services`):** zero-cost, 100% браузерные утилиты (HTML5 Canvas, Web Workers, JSZip), решающие узкую задачу мгновенно без нагрузки на backend и без передачи конфиденциальных данных пользователя на сервер.
  2. **Готовые решения (`/resheniya`):** пошаговый сквозной маршрут с контрактом результата, заранее выбранным стеком, готовыми командами и промптами.
  3. **Каталоги экосистемы (`/avito`, `/ai-tools`):** контекстный выбор проверенных внешних инструментов под задачу.
  4. **Бесшовная связка (Ecosystem Flywheel):** В готовом решении как обязательный шаг встроен нативный микросервис; на странице микросервиса есть прямой вход в готовое решение под ключ; в каталоге подсвечиваются оба. Это даёт непрерывный пользовательский опыт, высокий LTV, рост доверия и максимальный поисковый SEO-вес.

### 2. Запуск Хаба микросервисов (`/services`) и Админ-панели (`/admin/services`)
- Создана единая страница каталога `/services` с категоризацией, поиском, бейджами и карточками утилит.
- Добавлена модель `MicroserviceMeta` в Prisma: кастомные заголовки, описания, обложки, счетчики просмотров, запусков и шерингов.
- Созданы API-маршруты `/api/services/[slug]/stats` и `/api/admin/services`.
- В админке добавлен раздел «Микросервисы» (`/admin/services`) с возможностью редактирования метаданных и загрузки обложек.
- На главной странице (`/`) выведен новый динамический виджет «Микросервисы под задачи».

### 3. Avito Photo Lab (`/services/avito-photo-uniquizer`)
Полноценный инженерный конвейер уникализации и подготовки товарных фото для продавцов и авитологов:
- **Алгоритмическое ядро сходства:** pHash (Perceptual Hash), dHash (Difference Hash), расстояние Хэмминга и 64-битный анализ 3D RGB Cube (цветовая гистограмма).
- **Охранная область 4:3 и 1:1:** официальное горизонтальное соотношение Авито 4:3 с центральным защитным квадратом 1:1 для мобильных устройств, умный блюр-паддинг, центр-кроп и оверлеи реального UI мобильного приложения Авито.
- **Аппаратный EXIF Spoofer & AI Sanitizer:** генерация реалистичных тегов физических камер (Apple iPhone 15 Pro, Samsung Galaxy S24 Ultra, Xiaomi 14 Pro, Sony A7 IV) со сдвигом дат и зачисткой скрытых метаданных нейросетей (C2PA, IPTC AI, Midjourney, DALL-E, ComfyUI, SynthID).
- **Комбайн масс-постинга:** автосборка непересекающихся ракурсов под 10–50 объявлений с гарантией нулевого дублирования главных фото.
- **Генератор официальных XML / CSV фидов:** экспорт файла `avito_autoload_feed.xml` (`formatVersion="3"`) и CSV для Автозагрузки Авито Pro.
- **Сканер дубликатов:** попарная матрица схожести фото с подсветкой рискованных совпадений.

### 4. Новое готовое решение: «Запустить AI-магазин на Авито» (`/resheniya/avito-business`)
- Спроектирован и внедрён новый маршрут по правилам `resheniya-author`:
  - `environment`: Рабочее место и антидетект-безопасность.
  - `niche-research`: Аналитика спроса и LSI-семантики через `/avito`.
  - `ai-copywriting`: AI-копирайтинг офферов по конверсионной структуре.
  - `photo-lab`: Пакетная уникализация в Avito Photo Lab.
  - `feed-assembly`: Сборка XML/CSV фида Автозагрузки.
  - `deploy-and-schedule`: Публикация и расписание в Авито Pro.
  - `ai-lead-automation`: Подключение AI-автоответов в чатах.
- Создана обзорная страница `/resheniya/avito-business` и интерактивная рабочая зона `/resheniya/avito-business/workspace`.
- Пройдена автоматическая валидация `resheniya-validator` (PASS, 7 steps).

### 5. Сквозная перелинковка и SEO
- Зарегистрированы все роуты в `SITE_TREE` (`src/app/sitemap/site-map-data.ts`) и `sitemap.ts`.
- Валидатор `scripts/validate-sitemap.ts` завершился с кодом 0 (все страницы зарегистрированы).
- Добавлены кросс-ссылки между `/resheniya`, `/avito` и `/services/avito-photo-uniquizer`.
- Проект успешно собран (`next build`), PM2 перезапущен. Все роуты возвращают HTTP 200.
...
## 30.08.2026 — Проектирование голосового проводника ProektMap (Voice Guide)

### 1. Концепция голосового проводника (Voice Guide)
- Спроектирована архитектура аудиогида по разделам ProektMap (`/`, `/resheniya`, `/avito`, `/skills`, `/arsenal` и др.).
- Модель взаимодействия: деликатное предложение послушать краткую суть раздела (20–35 сек) один раз за сессию (`sessionStorage`).
- Три режима в настройках: «Спрашивать перед чтением» (по умолчанию), «Читать автоматически», «Выкл» (`localStorage`).
- По окончании аудио — блок «Что сделать дальше?» с кнопками быстрых переходов.

### 2. Архитектура генерации и доставки аудио
- **Принцип:** Статические заранее сгенерированные MP3 файлы в `/public/audio/guides/` (нулевая нагрузка на TTS во время визитов, мгновенный отклик, 100% стабильное воспроизведение на всех устройствах).
- **Слой Voice Script и фонетическая нормализация:** словарь транскрипции IT-терминов для естественного произношения (ProektMap → ПроектМэп, SaaS → саас, Next.js → Некст джи-эс, MCP → эм-си-пи, Prisma → Призма, API → эй-пи-ай, GitHub → Гитхаб).

### 3. Тестирование синтеза и выбор голоса
- Сгенерированы тестовые MP3-образцы нейросетевых голосов в `/public/audio/samples/`.
- Проведено сравнительное тестирование сырого текста vs фонетически нормализованного текста на IT-терминах стека ProektMap.
- Зафиксирован фаворит по подаче: бодрый темп (+6%), чистый дикторский тон без затягивания.
- Запланировано слепое сравнение Yandex SpeechKit (Ermil / Alena / Filipp / Dasha) vs Microsoft Neural vs ElevenLabs после подключения API-ключа Yandex Cloud.

---

## 31.08.2026 — Запуск «Голосового проводника» (Voice Guide) на ProektMap

### 1. Подключение Yandex SpeechKit и слепой тест
- Подключены сервисные ключи `YANDEX_API_KEY` и `YANDEX_FOLDER_ID` в `.env`.
- Сгенерированы образцы голосов: Алёна, Ермил, Филипп, Джейн, Омаж, Захар, Мадирус, а также Светлана и Дмитрий (Microsoft Neural).
- Выбран эталонный голос для проекта: **Yandex Алёна** (тёплая, естественная интонация).

### 2. Реестр сценариев и фонетическая нормализация
- Создан файл `src/lib/voice-guide/guide-data.ts` с реестром для 8 разделов (`/`, `/resheniya`, `/avito`, `/skills`, `/arsenal`, `/prompts`, `/vaibik`, `/services`).
- Добавлен словарь фонетической нормализации `IT_VOICE_DICTIONARY` и функция `normalizeForSpeech`.
- Для каждого раздела прописаны `durationSec`, `rawScript` (для субтитров), `voiceScript` (для диктора), `audioSrc` и быстрые действия `nextActions`.

### 3. Скрипт пакетной генерации
- Написан `scripts/generate-voice-guides.py` с поддержкой провайдеров Yandex SpeechKit (`--provider=yandex`) и Edge-TTS (`--provider=edge`).
- Добавлены команды в `package.json`: `npm run voice:guides` и `npm run voice:guides:edge`.
- Сгенерированы 8 оптимизированных MP3-файлов в `/public/audio/guides/`.

### 4. UI-компонент Голосового проводника (`VoiceGuideWidget`)
- Разработан легкий адаптивный виджет в `src/components/voice-guide/voice-guide-widget.tsx`:
  - Деликатный приглашающий баннер при первом входе в раздел (задержка 1.2 сек).
  - Развернутый плеер с анимацией звуковых волн, таймером, прогресс-баром и кнопками навигации.
  - Кнопка просмотра текста фразы (субтитры).
  - Настройки режима (Спрашивать / Читать автоматически / Выключен) с сохранением в `localStorage`.
  - Учет сессий в `sessionStorage` (один раз на раздел за визит).
- Виджет встроен в глобальный `layout.tsx`.

### 5. Билд, валидация и деплой
- Валидатор карты сайта: `npm run validate:sitemap` — успешно.
- Продуктовый билд: `npx next build` — успешно без ошибок.
- PM2 перезапущен. Роуты и аудиофайлы возвращают HTTP 200.

### 6. Документация и Skill для агентов
- Создана документация `docs/VOICE-GUIDE.md` с архитектурой, шаблонами сценариев и тарифами.
- Создан агентский навык `.cursor/skills/voice-guide-author/SKILL.md` для добавления аудиогидов в новые разделы.
- Обновлен `AGENTS.md` с фиксацией правил Голосового проводника.

---

## 31.08.2026 — Запуск микросервиса: «Конструктор голосового проводника для сайтов» (`/services/voice-guide-builder`)

### 1. Архитектура и функционал конструктора
- **Интерактивный конфигуратор маршрутов:** добавление страниц (URL, заголовок, текст 20–35 сек, диктор, кнопки действий CTA).
- **Живой тест синтеза речи:** мгновенное воспроизведение голоса прямо в браузере через API `/api/voice-guide/synthesize` (Microsoft Svetlana Neural +6% и Dmitry Neural +4% без API-ключей и оплат).
- **Готовые шаблоны:** SaaS / AI-сервис, Интернет-магазин / Авито, Агентство / Услуги.
- **Визуальная кастомизация & Live Preview:** выбор палитры (8 цветов + произвольный HEX), позиция (слева/справа), режимы поведения (Спрашивать / Авто) и интерактивный макет сайта на десктопе и смартфоне.

### 2. Встраиваемый универсальный скрипт (`/public/vguide/widget.js`)
- Легковесный автономный JS-виджет (0 зависимостей).
- Поддерживает установку в 1 строчку `<script>` на Tilda (блок T123), WordPress, Webflow, React / Next.js и обычные HTML-страницы.
- Учитывает сессии (`sessionStorage`) и выбор пользователя (`localStorage`).

### 3. Интеграция в экосистему ProektMap
- Зарегистрирован в каталоге `/services` и карте сайта `SITE_TREE`.
...
## 04.09.2026 — Claude Academy в продукте
- Компонент `ClaudeAcademyCallout`: Academy = бесплатная школа, ProektMap = полигон результата.
- Встроено на `/` (после «Как это работает») и `/resheniya` (перед каталогом маршрутов).
- Pillar-пост: `/blog/claude-academy-i-proektmap` (автор Алексей, `aiGenerated: false`).
- В шапку меню Academy не добавляли (внешняя ссылка только в контенте).

## 2026-09-04 — План нейроарсенала

### Что сделано
- Разобран Excel `public/АРСЕНАЛ НЕЙРОРАЗВЕДКИ (27.08.2026).xlsx`: **304** строки каталога, **298** уникальных имён, **6** дублей, **14** категорий с реальными счётчиками (голос 39, локальный AI 38, кодинг 37, агенты 37, изображения 31 и далее).
- Зафиксировано позиционирование: арсеналы = **стеки под миссию**, мост к `/resheniya`, не вторая свалка `/ai-tools`, MVP без SEO-фермы.
- Созданы документы:
  - `docs/ARSENAL-PLAN.md` — видение, UX, IA, поэтапный roadmap всех 304, трекер статусов, антипаттерны;
  - `docs/ARSENAL-STRUCTURE.md` — маршруты `/arsenal`, поля карточек, поиск/фильтры, SEO/SITE_TREE, меню только через админку, связь с экосистемой;
  - `docs/ARSENAL-V1-STACKS.md` — **12** первых арсеналов с реальными именами инструментов из Excel;
  - `docs/ARSENAL-TRACKER.md` — чеклист ✅/🟡/⬜.
- Этап 0 (план) отмечен как **сделано**; этап 1 (код MVP) — **запланировано**.

### Что намеренно НЕ сделано
- UI `/arsenal` не реализовывали (ждём одобрения).
- Коммит / push / деплой не делали.

### Следующий шаг
- После «да» от Алексея — этап 1: оболочка `/arsenal` + данные 12 стеков + SITE_TREE + меню через `/admin/menu`.

## 2026-09-04 — Нейроарсенал этап 1 (MVP)

### Что сделано
- Живой раздел `/arsenal`: хаб с поиском и фильтрами, 12 карточек арсеналов, курированный срез инструментов.
- Данные: `src/lib/arsenal/*` — 12 стеков из V1-STACKS + **102** инструмента из Excel (дедуп 5 имён).
- Маршруты: `/arsenal`, `/arsenal/[slug]`, `/arsenal/tools/[slug]` (DoD, типичная ошибка, внешние ссылки, мосты на `/resheniya` / `/avito` / `/services`).
- SEO: SITE_TREE, sitemap.ts (стеки + тулы), metadata/canonical, validate:sitemap.
- Меню: upsert `header-arsenal` → `/arsenal` через `scripts/sync-header-menu.ts`.
- Билд + PM2 + smoke HTTP 200.

### Что отложено
- Добор категорий Excel (этапы 2–9, ~204 инструмента).
- Админ CRUD / Prisma-модели.
- Voice Guide MP3 для `/arsenal` (маршрут уже в guide-data).

### Коммит
- Не делали (по запросу).

## 2026-09-04 — Нейро каталог: ребрендинг + этап 2 (Голос и Аудио)

### Что сделано
- Публичное имя раздела: **Нейроарсенал** → **Нейро каталог** (меню, H1, metadata, SITE_TREE, voice-guide registry, docs). URL `/arsenal` сохранён (без редиректов).
- Этап 2: добраны **27** инструментов категории 🎙️ Голос и Аудио из Excel → в каталоге **39/39** голос, всего **129** тулов.
- Трекер: этап 2 → **сделано**. Меню: `header-arsenal` label обновлён через sync-скрипт.
- Билд + PM2 + smoke `/arsenal`, `/resheniya`, `/vaibik`, `/avito`. Коммит не делали.

### Следующий шаг
- Этап 3: добор «Локальный AI и Модели» (~28 инструментов).


## 2026-09-04 — Нейро каталог: этап 3 (Локальный AI и Модели)

### Что сделано
- Этап 3: добраны **24** инструмента категории 🖥️ Локальный AI и Модели из Excel → в каталоге **38/38** local-ai, всего **153** тула.
- Дедуп по имени/slug против уже существующих 14 (Ollama, LM Studio, Jan и др.).
- Трекер: этап 3 → **сделано**. URL `/arsenal`, display name **Нейро каталог** без изменений.
- Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 4: добор «Кодинг и Разработка» (~26 инструментов).

## 2026-09-04 — Нейро каталог: этап 4 (Кодинг и Разработка)

### Что сделано
- Этап 4: добраны **21** инструмент категории 💻 Кодинг и Разработка из Excel → в каталоге **35**/35 уникальных (Excel 37 строк, дубли Agent Skills / Understand Anything уже были), всего **174** тула.
- Примеры: ZCode, OpenCodex, jcode, pen.dev, Stitch, Vibecraft, Caveman, Greenlight.
- Трекер: этап 4 → **сделано**. URL `/arsenal`, display name **Нейро каталог**.
- Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 5: добор «AI-агенты и Скиллы» (~26 инструментов).

## 2026-09-04 — Нейро каталог: этап 5 (AI-агенты и Скиллы)

### Что сделано
- Этап 5: добраны **17** инструментов категории 🧩 AI-агенты и Скиллы из Excel → в каталоге **37**/37 уникальных (Excel 37 строк, 0 дублей внутри категории), всего **191** тул.
- Уже были 20 (Hermes, Goose, Stagehand, Agent Reach и др.); Agent Reach — кросс-категорийный дубль с «Поиск», уже помечен `wasDuplicate`.
- Примеры: CopilotKit, DeerFlow, SkillsMP, Distilly, OpenGame, Application Skills.
- Трекер: этап 5 → **сделано**. URL `/arsenal`, display name **Нейро каталог**.
- Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 6: добор «Изображения и Графика» (~22–25 инструментов).

## 2026-09-04 — Нейро каталог: этап 6 (Изображения и Графика) + hero-статы

### Что сделано
- Этап 6: добраны **18** инструментов категории 🎨 Изображения и Графика из Excel → в каталоге **30**/30 уникальных (Excel 31 строка, дубль Real-ESRGAN уже был), всего **209** тулов.
- Примеры: Stable Diffusion 1.5/XL, OpenCV, Dreamina, Mint, OpenPencil, Genspark AI Designer, Колоризатор фото (Mail.ru).
- Hero mini-dashboard на `/arsenal`: 4 живых счётчика из `getArsenalHubStats()` — арсеналы, инструменты, категории с материалом, карточки со ссылкой (сайт/скачать). Сетка 2×2 на мобиле, ряд на десктопе.
- Трекер: этап 6 → **сделано**. URL `/arsenal`, display name **Нейро каталог**.
- Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 7: добор «Видео и Анимация» + «Маркетинг, SEO и SMM» (~29 инструментов).

## 2026-09-04 — Нейро каталог: этап 7 (Видео + Маркетинг)

### Что сделано
- Этап 7: добраны **13** инструментов 🎬 Видео и Анимация → **20**/20 уникальных (Excel 21 строка, дубль HyperFrames уже был) и **7** инструментов 📈 Маркетинг, SEO и SMM → **8**/8; всего **229** тулов.
- Примеры video: Wisecut, Google Vids, VideoLingo, Palmier Pro, Cutback, VideoSOS, OpenMontage, Reactor.
- Примеры marketing: OpenSEO, Virality Predictor, Maxfusion AI, Draper, geo-seo-claude.
- Hero-статы авто из данных: 12 арсеналов / 229 инструментов / 11 категорий / 229 со ссылкой.
- Трекер: этап 7 → **сделано**. URL `/arsenal`. Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 8: добор «Поиск и Исследования» + «Промпты» + «LLM» (~39 инструментов).

## 2026-09-04 — Нейро каталог: этап 8 (Поиск + Промпты + LLM)

### Что сделано
- Этап 8: добор из Excel — 🔍 **+5** → **13**/13 уникальных (карточек research **12** + Agent Reach в agents), 💬 Промпты **+3** → **11**/12 (jailbreak-строка отфильтрована по плану §7.5), 💬 LLM **+9** → **14**/14; всего **+17**, каталог **246**.
- Примеры research: OpenMed, GeoSpy, MiroFish, Sci-Bot, Gemini Notebook.
- Примеры prompts: Awesome Nano Banana Pro / Images, auto-improve.
- Примеры LLM: Qwen, Qwen Studio, Poe, Kimi K2.6, Z.ai, Meta AI, Voyager, Tolaria.
- Hero-статы авто: 12 арсеналов / 246 инструментов / 11 категорий / 246 со ссылкой.
- Трекер: этап 8 → **сделано**. Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 9: добор «Обучение и Гайды» + «Презентации и Документы» + «FinTech» + «Разное» (~54).

## 2026-09-04 — Нейро каталог: этап 9 (Обучение + Docs + FinTech + Разное)

### Что сделано
- Этап 9: добор из Excel — 🧠 **+12** → **14**/14 уникальных (Excel 15/−1 дубль AI Engineering; Easy-Vibe и Zero to Claude Code уже были), 📊 **+9** → **9**/9, ₿ **+6** → **6**/6, 📁 **+20** → **20**/24; всего **+47**, каталог **293**.
- Отфильтровано из «Разное»: AWESOME OSINT ARSENAL, HuggingBay, BookHunter, GPTDisguise (план §7.5).
- В фильтры хаба добавлены категории docs / fintech / misc.
- Примеры: Learn AI, Claude Academy, Beautiful.ai, Presenton, Danelfin, MetaMask Agent Wallet, Browsery Tools, CapWords.
- Hero-статы авто: 12 арсеналов / 293 инструментов / 14 категорий / 293 со ссылкой.
- Трекер: этап 9 → **сделано**. Билд + PM2 + smoke. Коммит не делали.

### Следующий шаг
- Этап 10: дедуп + QA + перелинковка `/arsenal` ↔ `/resheniya`.

## 2026-09-04 — Нейро каталог: этап 10 (дедуп + QA + перелинковка)

### Что сделано
- **Дедуп:** подтверждены 6 Excel-дублей (`wasDuplicate`); слит near-dup `awesome-nano-banana-pro` → `awesome-nano-banana-pro-prompts`. Каталог **292**.
- **Исключения:** `docs/ARSENAL-EXCLUSIONS.md` (jailbreak + 4 misc по §7.5).
- **QA URL:** скрипт `scripts/qa-arsenal-urls.ts`; фиксы QClaw→`qclaw.qq.com`, MetaMask→docs, Hermes Skills, Krea homepage. ElevenLabs с RU — geo-redirect (не битая ссылка).
- **Перелинковка:** `NeuroCatalogCallout` на `/resheniya`; `ArsenalBridgePanel` на saas / telegram / avito; `RESHENIYA_ARSENAL_BRIDGES`; усилены `relatedRoutes` стеков; хаб `/arsenal` → конкретные решения.
- **Skill:** `.cursor/skills/arsenal-resheniya-bridge/SKILL.md` (trust: community); чеклист в `resheniya-author` (5b) и soft-warn в `resheniya-auditor` (D2).
- Трекер: этап 10 → **сделано**. Билд + PM2 + smoke. Коммит не делали.

### Каталог
MVP-пайплайн этапов 0–10 закрыт. Display name **Нейро каталог**, URL `/arsenal`.

## 2026-09-04 — Финал: ссылки на Каталог ИИ + Stage complete

### Что сделано
- Главная: в блоке «Создавайте работающие продукты вместе с AI-инженерами» (`CommunityPulseHero`) добавлена карточка **Каталог ИИ** → `/arsenal` (счётчик из `getArsenalHubStats`).
...
## 2026-09-06 — Авто-сбор + «Популярное за неделю» + аудит трафика

### Авто-сбор блога
- Тихий skip при полной очереди (≥ лимит×2) — без Telegram-шума.
- Один слот утра/вечера: повтор в тот же час МСК не стартует.

### Главная
- Виджет «Популярное за неделю» — посты за 7 дней по viewCount (не all-time); добор из 14 дней если мало.

### Аудит трафика
- Разбор низкой посещаемости: продукт шире аудитории, авто-блог без дистрибуции, слабая видимость в поиске.
- Canvas: traffic-low-diagnosis (локально в Cursor).

## 2026-09-06 — Закрытие дня (вечер)
...
## 08.09.2026 — Правка честности: границы Figma MCP в маршруте дизайнера

### Зачем
Анализ показал риск ввести в заблуждение: официальный Figma MCP — чтение и Dev Mode,
запись макетов в файл Figma он не поддерживает, а маршрут обещал «перенос в Figma слоями» и «создание макетов агентом».

### Сделано
- Шаг «MCP + Figma»: явные границы — чтение/Dev Mode, запись — плагином импорта или руками; тариф Professional/Dev, лайфхак Figma for Education
- Шаг «Прототип → Figma»: разделение ролей — агент готовит структуру и чек-лист, пользователь переносит (плагин/руками), агент сверяет с системой; промпт переписан
- Шаги «Наполнение» и «Аудит»: «собирает экраны внутри системы» → «читает через MCP и опирается на систему в прототипе/коде»
- Страница: герой, «контракт результата», подписи скриншотов 01/04 — без обещания записи в Figma

### Проверка
- `next build` — OK (32.4s); `/resheniya/designer-agent` + `/workspace` — HTTP 200
- Новые формулировки видны в HTML страницы и рабочей зоны
- `git status` чистый

### Точка отката
- Коммит `c9e7f025` на `main` = `master`, push origin

### Открытые решения (не выбраны пользователем)
- Блок «что понадобится» (терминал, Node, подписка) наверху маршрута
- Отделение карточки дизайнера от «запуска продуктов» в хабе
...
## 16.09.2026 — Шпаргалка `/shpargalka` (волны 1–2)

### Зачем
Нужна русская коллекция готовых промптов по профессиям: скопировал шаблон, подставил скобки, отправил в чат. Это не Skills (`/ai-skills`) и не инженерные System Prompt (`/prompts`).

### Сделано
- Новый раздел **Шпаргалка**: хаб `/shpargalka` + паки `/shpargalka/[slug]`
- 16 опубликованных паков, **338** оригинальных русских шаблонов
- Волна 1: разработчик, дизайнер, маркетолог, контент, Excel, поиск работы, образование, HR, предприниматель, сайты
- Волна 2: юрист, бухгалтер, журналист, финансы, книги, коучи (черновики/вопросы специалисту, не консультация)
- Расширение **Разработчик** (30): открытие/закрытие дня, старт проекта, `AGENTS.md`, философия, `REFERENCES.md` («смотри это — не делай то»), handoff
- Расширение **Дизайнер** (28): анти-slop чеклист, референсы, негативный визуал, красные флаги AI-макета
- Учебный слой: плохо → хорошо + before/after + кнопка Copy
- Меню шапки: `header-shpargalka` через БД (`scripts/sync-header-menu.ts`)
- Перелинковка: `/ai-skills`, `/prompts`, поиск сайта, Нейро каталог `prompt-ops`

### SEO / карта сайта
- `SITE_TREE`: хаб + 16 детей (включая волну 2)
- `sitemap.ts`: `getPublishedPacks()` + priority 0.8
- `validate:sitemap` — PASS
- Живые `/sitemap` и `sitemap.xml`: `/shpargalka` и все паки на месте
- Метаданные: `metadata` на хабе, `generateMetadata` на паках

### Проверка
- `next build` — OK; `pm2 restart proektmap` — online
- HTTP 200: `/shpargalka`, паки волн 1–2, `/resheniya`, `/vaibik`, `/avito`
- Фильтры `день` / `анти-slop` на живых страницах работают

### Точка отката
- Коммит на `main` = `master`, push origin

### Следующие шаги
- Волна 3 по желанию (продажи, поддержка, продукт и т.п.)
- Голосовой гид на `/shpargalka`, если понадобится
...
## 17.09.2026 — Конструктор стиля сайта + хаб/спица

### Зачем
Нужен личный рабочий съём стиля: вставил URL → токены, DESIGN.md и промпт агенту с уже прописанными GitHub-скиллами. Не клон сайта и не учебник. Превью должно показывать дизайн-систему этого сайта, а не школу из каталога.

### Сделано
- Микросервис `/services/site-style-builder`: URL → Playwright CSS → DeepSeek-досье (фон, шрифты, акцент, радиусы, сетка)
- Превью «Система»: палитра, type, радиусы, атомы, сетка
- В DESIGN.md и промпт вшит пакет: Frontend Design, taste-skill, web-design-guidelines, Impeccable
- Исправление Altea (`https://altea.clinic`): белый холст `#FFFFFF`, шрифт Onest, без Times New Roman и кремовой «бумаги журнала»
- Если DeepSeek молчит — CSS-досье всё равно держит веб-шрифт и холст страницы
- Правило хаб/спица: skill `hub-spoke-author`, новые разделы садятся на одну из 4 станций шапки, не новым корнем

### SEO / карта сайта
- `SITE_TREE`: конструктор в UI-Атласе и в микросервисах
- Метаданные через `/services/[slug]`

### Проверка
- `next build` — OK; `pm2 restart proektmap` — online
- HTTP 200: `/services/site-style-builder`, `/resheniya`, `/vaibik`, `/avito`
- Живой съём Altea: Onest + белый фон + акцент `#226DB7`

### Точка отката
- Коммит на `main` = `master`, push origin

### Следующие шаги
- Съём по скриншоту (v2)
- Голосовой гид и обзор в блоге — по желанию
...
## 18.09.2026 — Разведка libmdbx / граф знаний (без внедрения)

### Зачем
Оценить https://libmdbx.dqdkfa.ru и `/knowledge` как возможный вклад в хаб ProektMap (арсенал, VibeCraft, паттерн базы знаний).

### Сделано
- Просмотрен проект libmdbx: встраиваемый KV (наследник LMDB), экосистема Ethereum/Isar/биндинги
- Разобран граф знаний `/knowledge`: ~321 узел, ~2700 рёбер, типы факт/рассуждение/вывод/правило/чек-лист/API/источник; данные в `/static-content/knowledge-graph.json`
- Связка с хабом: сайт собран через VibeCraft (релевантно `/vibecraft` и `/vaibik`); паттерн графа близок к `docs/KNOWLEDGE-GRAPH-BLOG.md`
- Решение: **пока не внедрять** — зафиксированы точки входа на будущее (кейс VibeCraft, карточка `/arsenal`, опционально маленький граф на станции хаба)

### SEO / карта сайта
- Без изменений публичных страниц

### Проверка
- Только внешний ресерч; код и деплой не трогали

### Точка отката
- Без кода; только запись в DEVLOG

### Следующие шаги
- По запросу: кейс на `/vibecraft`, карточка libmdbx в `/arsenal`, или пилот своего графа на одной станции

### Закрытие дня
День закрыт. Коммит + push. main = master.

## 25.09.2026 — Автопилот контент-цеха V2 + медиа-витрина главной

### Зачем
Перейти от «фабрики RSS-новостей» к поисковым активам по матрице спроса (1 guide/сутки, ядро без Авито) и сделать главную живым медиа-лицом: свежие гайды/видео/микросервисы, сетка хаба, цифры разделов.

### Сделано
- **Content Autopilot V2:** `SiteSettings.contentAutopilot*`, `src/lib/blog/content-engine/*` (матрица, pick → DeepSeek → гейт → publish → TG), API `/api/blog/content-autopilot`, cron `autopilot-wrapper.sh`, рубильник в `/admin/blog/settings`; при включённом автопилоте legacy news drip останавливается
- Документация: `docs/content-engine/PHASE-0.md`; артефакты Phase 0 в `data/content-engine/`
- Проверено: пятничный дайджест бота читает RSS блога — гайды автопилота попадут в выжимку без переделки бота
- **Медиа-витрина главной:** `HomeMediaWall` (блог + VK Video + микросервисы с превью), `HomeHubDirectory` (спицы 4 станций), `HomeMetricsBar` (тёмная полоса со счётчиками из БД/данных)
- Подписи анонсов **под** фото (не поверх инфографики обложек)

### SEO / карта сайта
- Новый публичный URL не добавлялся; `/` обновлён содержимым. Меню шапки без нового корня.

### Проверка
- `next build` — OK; `pm2 restart proektmap` — online
- HTTP 200: `/` (блоки «Свежее на карте», «Разделы карты», «Карта в цифрах»), smoke автопилота ранее публиковал гайды в `/blog`

### Точка отката
- Коммит на `main` = `master`, push origin

### Следующие шаги
- По желанию: deep-link видео с главной; weekly TG-отчёт матрицы; подкрутить промпт пятничного дайджеста под гайды (меньше упор на Авито)
