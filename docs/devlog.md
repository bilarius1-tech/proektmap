...
## 2026-08-01 — Фиксы и стабилизация

### AI Цех — фиксы
- **Меню**: AI Цех вынесен из Песочницы в отдельный пункт главного меню (🏭 AI Цех)
- **Админка**: добавлена ссылка на AI Цех в боковом меню админки (иконка Factory)
- **AI-fill**: полный рефакторинг парсинга URL
  - Чистка query params (`?utm_source=...`) и `.git` из URL
  - Понятные ошибки: «Репозиторий не найден», «Лимит запросов GitHub API», «Недостаточно текста»
  - Fallback: если GitHub API не сработал → прямая загрузка страницы
  - Обработка markdown-блоков в ответе AI (```json ... ```)

### Проверка AI-агентов
- DeepSeek API ключ жив и работает (из siteSettings.deepseekApiKey)
- Блог AI-enhance и auto-publish используют тот же ключ → работают

### Закрытие дня
- Документация: SANDOBOX.md, AI-WORKSHOP.md
- Песочница: 6 страниц (sandbox, telegram, ai-without-vpn, vibecraft, russian-ai-stack, ai-workshop)
- 7 коммитов за день
